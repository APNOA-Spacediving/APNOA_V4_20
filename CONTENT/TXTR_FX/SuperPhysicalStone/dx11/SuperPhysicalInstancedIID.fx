...
)	
{
	...
	float4x4 wo = world[ii + IntanceStartIndex];
	
	/* the WORLD transform applies to the 
	whole batch in case of instancing, so we can transform 
	all the batch at once using it */
	wo = mul(world[ii + IntanceStartIndex],tW);
	
	
	Out.PosW = mul(PosO, wo);
	Out.PosWVP = mul(PosO,mul(wo,tVP));
	Out.NormW = normalize( mul(mul(NormO, (float3x3)wo), (float3x3)transpose(tWI)) );
...
#ifdef doShadowPOM
float4 doLighting(float4 PosW, float3 N, float4 TexCd, float2 pos, float3x3 tbn,uint texID){
#else
float4 doLighting(float4 PosW, float3 N, float4 TexCd, float2 pos, uint texID){
#endif
				...
				if(Light[i].useShadow){
					...
					for(int p = 0; p < 6; p++){
						...
						&& (saturate(projectTexCoord.z) == projectTexCoord.z)){
						    ...
						    float attenuation = Light[i].lAtt0 * falloff;
							finalLight += cookTorrance(V, L, N, albedo.xyz, Light[i].Color.rgb,
							shadow.x, 1, lightDist, Material[texID].sssAmount, Material[texID].sssFalloff, F0, attenuation, roughness, metallic, ao, iridescenceColor, texID);
...
{	
	uint texID = materialID[In.ii + IntanceStartIndex];
	#ifdef doShadowPOM
		return doLighting(In.PosW, In.NormW, In.TexCd, pos.xy, 1, texID);
	#else	
		return doLighting(In.PosW, In.NormW, In.TexCd, pos.xy, texID);
	#endif
}
...
{	
	...
	if(Material[texID].POM && useTex[texID]){
		parallaxOcclusionMapping(In.TexCd.xy, In.PosW.xyz, normalize(tVI[3].xyz - In.PosW.xyz), float3x3(In.tangent,In.binormal,In.NormW.xyz),texID, 0);
	}
	#endif
	...
	#ifdef doShadowPOM
		return doLighting(In.PosW, Nb, In.TexCd, pos.xy, float3x3(In.tangent, In.binormal,In.NormW), texID);
	#else	
		return doLighting(In.PosW, Nb, In.TexCd, pos.xy, texID);
	#endif
...
//	float3 n = normalize(In.NormW);
	...
	#ifdef doPOM
	if(Material[texID].POM && useTex[texID]){
		parallaxOcclusionMapping(In.TexCd.xy, In.PosW.xyz, normalize(tVI[3].xyz - In.PosW.xyz), float3x3(t,b,In.NormW.xyz),texID, 0);
	}
	#endif
	#endif
	...
	#ifdef doShadowPOM
		return doLighting(In.PosW, Nb, In.TexCd, pos.xy, float3x3(t, b, Nb), texID);
	#else	
		return doLighting(In.PosW, Nb, In.TexCd, pos.xy, texID);
	#endif
}
